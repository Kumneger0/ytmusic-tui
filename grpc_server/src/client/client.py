...
def convert_to_netscape_cookies(cookie_text: str) -> str:
    ...
    lines: list[str] = [line.strip() for line in cookie_text.splitlines() if line.strip() and not line.strip().startswith("#")]
    if lines and all(len(line.split("\t")) >= 7 for line in lines):
        return f"# Netscape HTTP Cookie File\n\n{cookie_text}"

    cookie_header_str: str = cookie_text
    try:
        parsed_data: JSONValue = cast(JSONValue, json.loads(cookie_text))
        if isinstance(parsed_data, dict):
            raw_cookie = parsed_data.get("Cookie") or parsed_data.get("cookie")  
            if isinstance(raw_cookie, str):
                cookie_header_str = raw_cookie
            else:
                headers_obj = parsed_data.get("headers")
                if isinstance(headers_obj, dict):
                    h_cookie = headers_obj.get("Cookie") or headers_obj.get("cookie")
                    if isinstance(h_cookie, str):
                        cookie_header_str = h_cookie
        elif isinstance(parsed_data, list):
            cookie_netscape_lines: list[str] = [
            ...
            for item in parsed_data:
                if isinstance(item, dict):
                    ...
                    if name and value:
                        domain_val = item.get("domain", ".youtube.com")
                        domain = str(domain_val) if domain_val is not None else ".youtube.com"
                        http_only = bool(item.get("httpOnly", False))
                        if http_only and not domain.startswith("#HttpOnly_"):
                            domain = f"#HttpOnly_{domain}"
                        flag = "TRUE" if domain.startswith(".") or domain.startswith("#HttpOnly_.") else "FALSE"
                        path_val = item.get("path", "/")
                        path = str(path_val) if path_val is not None else "/"
                        secure = "TRUE" if bool(item.get("secure", True)) else "FALSE"
                        expiry_val = item.get("expirationDate") or item.get("expiry") or item.get("expires") or 2147483647
                        try:
                            expiry = int(str(expiry_val))
                        except (ValueError, TypeError):
                            expiry = 2147483647
                        cookie_netscape_lines.append(f"{domain}\t{flag}\t{path}\t{secure}\t{expiry}\t{name}\t{value}")
            return "\n".join(cookie_netscape_lines)
    except (json.JSONDecodeError, TypeError, ValueError):
        pass

    netscape_lines: list[str] = [
        "# Netscape HTTP Cookie File",
        "# https://curl.se/docs/http-cookies.html",
        "# This file was generated by clispot",
        "",
    ]
    raw_str: str = re.sub(r"^(cookie|Cookie):\s*", "", cookie_header_str.strip())
    items: list[str] = re.split(r"[;\r\n]+", raw_str)
    for item_str in items:
        clean_item: str = item_str.strip()
        if not clean_item or "=" not in clean_item:
            continue
        parts: list[str] = clean_item.split("=", 1)
        cookie_name: str = parts[0].strip()
        cookie_val: str = parts[1].strip()
        if not cookie_name:
            continue
        domain_name: str = ".youtube.com"
        sub_flag: str = "TRUE"
        cookie_path: str = "/"
        sec_flag: str = "TRUE"
        exp_time: str = "2147483647"
        netscape_lines.append(f"{domain_name}\t{sub_flag}\t{cookie_path}\t{sec_flag}\t{exp_time}\t{cookie_name}\t{cookie_val}")

    return "\n".join(netscape_lines)


def create_cookie_file(cookie_text: str) -> str:
    netscape_content: str = convert_to_netscape_cookies(cookie_text)
...
class MusicClient:
    ...
    def execute(self, func: Callable[[YTMusic], T]) -> T:
        ...
        try:
            ...
            return func(self.get_client())
    def get_stream_url_and_duration(self, video_id:str, user_cookie: str | None)  -> GetStreamURLResponse:
    ...
    def get_track(self, video_id: str, user_cookie: str | None = None) -> YTSongResponse:
        ...
        if isinstance(track_video_id, str):
            try:
                stream_url_and_duration = self.get_stream_url_and_duration(video_id=track_video_id, user_cookie=user_cookie)
